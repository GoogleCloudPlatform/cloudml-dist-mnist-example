...
for  i in $(seq 0 $NUM_PS); do
  ...
  gcloud compute ssh ps-${i} -- pkill -f trainer/task_p3.py
...
for  i in $(seq 0 $NUM_WORKER); do
  ...
  gcloud compute ssh worker-${i} -- pkill -f trainer/task_p3.py
  gcloud compute ssh worker-${i} -- pkill -f trainer/task.py
done

# Stop a master
echo "Terminating master-0..."
gcloud compute ssh master-0 -- pkill -f trainer/task_p3.py
